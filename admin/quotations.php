...
$params = [];
if ($search) {
    $conds[]  = '(q.quotation_number LIKE ? OR p.company_name LIKE ?)';
    $like     = '%' . $search . '%';
    $params[] = $like;
    $params[] = $like;
}
if ($date_from) {
    $conds[]  = 'DATE(q.created_at) >= ?';
    $params[] = $date_from;
}
if ($date_to) {
    $conds[]  = 'DATE(q.created_at) <= ?';
...
<div class="admin-layout">
  ...
  <main class="admin-main">
    <h1>Quotations</h1>
    <form method="GET" class="d-flex gap-2 mb-2" style="flex-wrap:wrap;align-items:center">
      <input type="text" name="q" class="form-control" style="max-width:220px" placeholder="Search quotation # or party…" value="<?= e($search) ?>">
      <input type="date" name="date_from" class="form-control" style="max-width:160px" value="<?= e($date_from) ?>" title="From date">
      <input type="date" name="date_to"   class="form-control" style="max-width:160px" value="<?= e($date_to) ?>"   title="To date">
      <button type="submit" class="btn btn-outline btn-sm">Filter</button>
      <?php if ($search || $date_from || $date_to): ?><a href="quotations.php" class="btn btn-sm" style="color:#888">Clear</a><?php endif; ?>
    </form>
    ...
    <div class="pagination mt-2">
      <?php for ($i=1;$i<=$pages;$i++): ?>
        <?php
          $qs = http_build_query(array_filter(['q'=>$search,'date_from'=>$date_from,'date_to'=>$date_to,'page'=>$i]));
        ?>
        <a href="?<?=$qs?>" class="<?=$i===$page?'current':''?>"><?=$i?></a>
      <?php endfor; ?>
    </div>
    <?php endif; ?>
  </main>
